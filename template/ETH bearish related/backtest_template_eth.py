...
try:
    from template.model_development_template_eth import compute_window_weights, precompute_features
    from template.prelude_template_eth import (
    ...
    from model_development_template_eth import compute_window_weights, precompute_features
    from prelude_template_eth import (
        backtest_dynamic_dca,
        check_strategy_submission_ready,
        load_data,
        parse_window_dates,
...
def run_full_analysis(
    ...
    check_strategy_submission_ready(btc_df, compute_weights_fn, features_df=features_df)
...
def main():
    ...
    os.chdir("/Users/mig/Documents/LSE/Capstone Project/lse-bitcoin-analytics-capstone-template/template/ETH bearish related")
    signal = pd.read_csv("eth_bearish.csv", index_col=0).iloc[:, 0]
    signal.index = pd.to_datetime(signal.index, format='mixed', dayfirst=True)
    signal.name = "signal"

    logging.info("Precomputing features...")
    _FEATURES_DF = precompute_features(btc_df, signal)

    base_dir = Path(__file__).parent.parent
    output_dir = base_dir / "output ETH bearish"

    df_spd, metrics = run_full_analysis(
        btc_df=btc_df,
