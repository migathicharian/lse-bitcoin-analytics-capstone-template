"""Dynamic DCA weight computation using signal strategy.
...
def precompute_features(df: pd.DataFrame, signal: pd.Series = None) -> pd.DataFrame:
    ...
    features = pd.DataFrame(
        {
            PRICE_COL: price,
            "signal": signal.shift(1).fillna(0),  # Lag 1 day
        },
        index=price.index,
    )
...
def compute_dynamic_multiplier(signal: np.ndarray) -> np.ndarray:
...
def compute_weights_fast(
    ...
    signal = _clean_array(df["signal"].values)
