...
try:
    from template.model_development_template_eth import precompute_features
except ImportError:
    from model_development_template_eth import precompute_features
...
import_signal = "eth_bullish.csv"

signal = pd.read_csv(import_signal, header = 0)
signal.name = 'signal'
...
def check_strategy_submission_ready(dataframe: pd.DataFrame, strategy_function, features_df=None) -> None:
    ...
    spd_table = compute_cycle_spd(dataframe, strategy_function, features_df=features_df)
    underperf = spd_table[
