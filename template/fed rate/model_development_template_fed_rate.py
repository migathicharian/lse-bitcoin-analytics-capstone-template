"""Dynamic DCA weight computation using fed rate strategy.

This module computes daily investment weights for a Bitcoin DCA strategy
based on a fed signal
"""
...
def precompute_features(df: pd.DataFrame, fed_signal: pd.Series = None) -> pd.DataFrame:
    ...
    features = pd.DataFrame(
        {
            PRICE_COL: price,
            "fed_signal": fed_signal.shift(1).fillna(0),  # Lag 1 day
        },
        index=price.index,
    )
...
def compute_weights_fast(
    ...
    fed_signal = _clean_array(df["fed_signal"].values)

    # Compute dynamic weights
    dyn = compute_dynamic_multiplier(fed_signal)
    raw = base * dyn
